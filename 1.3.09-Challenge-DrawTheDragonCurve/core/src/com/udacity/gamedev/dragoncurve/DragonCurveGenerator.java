...
public class DragonCurveGenerator {
    ...
    static LinkedList<Direction> dragonTurns(int recursions){
        ...
        for (int i = 0; i < recursions; i++){
            ...
            LinkedList<Direction> reversed = new LinkedList<Direction>(turns);
            Collections.copy(turns, reversed);
    ...
    static float[] generateDragonCurve(int width, int height, int recursions){
        ...
        for (Direction dir : turns) {
            heading = Direction.turn(heading, dir);
            head.x += heading.x;
            head.y += heading.y;
            curve[i++] = head.x;
            curve[i++] = head.y;
